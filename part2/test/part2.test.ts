...
import { lazyMap, lazyFilter, asycMemo, getAll, makePromisedStore, MISSING_KEY, asyncWaterfallWithRetry } from '../src/part2';
...
describe('2.1 (PromisedStore)', () => {
    ...
    it('two insertions', async() => {
        const store = makePromisedStore();
        await store.set('a', 42);
        await store.set('b', 44);
        const a = await store.get('a');
